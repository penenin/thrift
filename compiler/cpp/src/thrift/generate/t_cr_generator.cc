...
public:
  t_cr_generator(t_program* program,
    ...
    for( iter = parsed_options.begin(); iter != parsed_options.end(); ++iter) {
      if( iter->first.compare("namespaced") == 0) {
        namespaced_ = true;
      } else {
        throw "unknown option crystal:" + iter->first;
      }
    }
...
/**
 * Renders the require of thrift itself, and possibly of the crystalgems dependency.
 */
  ...
  for (auto include : includes) {
    if (namespaced_) {
      ...
      std::string included_require_prefix
          = cr_namespace_to_path_prefix(included->get_namespace("cr"));
...
void t_cr_generator::generate_enum(t_enum* tenum) {
  ...
  for (c_iter = constants.begin(); c_iter != constants.end(); ++c_iter) {
    ...
    // crystal class constants have to be capitalized... omg i am so on the fence
...
t_cr_ofstream& t_cr_generator::render_const_value(t_cr_ofstream& out,
  ...
  if (type->is_base_type()) {
    ...
    case t_base_type::TYPE_I8:
      out << value->get_integer() << "_i8";
      break;
    case t_base_type::TYPE_I16:
      out << value->get_integer() << "_i16";
      break;
    case t_base_type::TYPE_I32:
      out << value->get_integer();
      break;
    case t_base_type::TYPE_I64:
      out << value->get_integer() << "_i64";
      break;
...
/**
 * Converts the parse type to a crystal tyoe
 */
